...
public class Appview {
	...
	public void createMaps(){
		...
		this.colorBucketList = new ArrayList();
	...
	public class BucketActionListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (appGame.gamemode == 0){
				...
				System.out.println("Win State is " + winState);
				if (winState == 0){					
					int turnsLeft = appGame.getTurnsLeft();
				...
				if (c != appGame.getCurrentPlayer().getOpponent().getCurrentColor()){
					...
					if (appGame.gamemode == 1) {
						...
						if (aiC == myColor || aiC == theirColor) {
							...
							aiC = availableColors.get(0);
						}
						System.out.println(aiC);
						int aiWinState = appGame.onClickFlood(aiC);
						updateFloodDisplay(aiC);
						if (aiWinState == 0){
							updateScoreDisplay();
							appGame.changePlayer();
						}
